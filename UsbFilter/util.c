...
//		#pragma alloc_text(PAGE, InitBufferQueue)
//		#pragma alloc_text(PAGE, InsertDataToQueue)
//		#pragma alloc_text(PAGE, PopDataFromQueue)
#endif
...
NTSTATUS QueryDeviceKey (IN HANDLE Handle, IN PWCHAR ValueNameString, OUT PVOID Data, IN ULONG DataLength)
    ...
    if (fullInfo) 
		{
        status = ZwQueryValueKey (Handle, &valueName, KeyValueFullInformation, fullInfo, length, &length);
...
VOID RegistryAccessSample(struct DEVICE_EXTENSION *devExt, PDEVICE_OBJECT devObj)
    ...
    if (NT_SUCCESS(status))
        ...
        if (NT_SUCCESS(QueryDeviceKey(hRegDevice, L"SampleFilterParam", &value, sizeof(value))))
			{
            //
        ...
        if (NT_SUCCESS(QueryDeviceKey(hRegDevice, L"SampleFilterParam2", &otherValue, sizeof(otherValue)))) 
			{
            //
            // Perform whatever operation is necessary 
            //
